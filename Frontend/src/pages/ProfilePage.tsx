...
const ProfilePage = () => {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await updateUser({ name, mobile, platform, city });
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 animate-fade-in max-w-2xl">
      ...
      <div className="rounded-lg border bg-card p-6 shadow-card space-y-4">
        ...
        <div><label className="text-sm font-medium">Mobile Number</label><Input value={mobile} onChange={e => setMobile(e.target.value)} /></div>
        <div><label className="text-sm font-medium">Platform</label><Input value={platform} onChange={e => setPlatform(e.target.value)} /></div>
        <div><label className="text-sm font-medium">City</label><Input value={city} onChange={e => setCity(e.target.value)} /></div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          ...
          <div className="flex items-center gap-3 rounded-lg bg-muted p-3">
            ...
            <div>
              ...
              <p className="font-medium">{user?.platform || 'Not set'}</p>
            </div>
          </div>
          <div className="flex items-center gap-3 rounded-lg bg-muted p-3">
          ...
          <div className="flex items-center gap-3 rounded-lg bg-muted p-3">
            ...
            <div>
              ...
              <p className="font-medium">{user?.workingHours || 'Not set'}</p>
            </div>
          </div>
          <div className="flex items-center gap-3 rounded-lg bg-muted p-3">
            <Mail className="h-4 w-4 text-muted-foreground" />
